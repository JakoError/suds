...
def bbox_image_transform(hparams):
    ...
    for camera_index in range(hparams.camera_num):
        for img_path in tqdm.tqdm(get_kitti_rendered_image_paths(hparams.render_path, camera_index=camera_index)):
...
def get_opts() -> Namespace:
    ...
    # parser.add_argument('--label_files', type=str, required=True)
